...
function byDateAndAlphabeticalFolderFirst(_cfg) {
  return (f1, f22) => {
    ...
    if (!f1IsFolder && f2IsFolder) return 1;
    // short_title 기준 정렬 (DVT-01, DVT-02 ... 순서)
    const s1 = f1.frontmatter?.short_title ?? f1.frontmatter?.title ?? "";
    const s2 = f22.frontmatter?.short_title ?? f22.frontmatter?.title ?? "";
    return s1.localeCompare(s2, "ko", { numeric: true });
  };
}
