...
**TLC가 정상 예측치의 80% 미만**이면 restrictive physiology를 의심한다.

TLC를 정확히 측정하려면 잔기량(residual volume, RV)까지 측정할 수 있는 전폐기능 검사가 필요하다. 간이 폐기능 검사로는 측정이 어렵다.

### Restrictive 패턴의 원인 — 네 가지 범주

1. **폐실질(lung parenchyma)**: 폐 조직 자체의 이상. 간질성 폐질환(ILD) 등.
2. **신경근육(neuromuscular)**: 폐와 기도는 정상이지만, 호흡근을 움직이는 신경이나 근육에 문제. 근위축성 측삭경화증(ALS), 근병증 등.
3. **흉벽(chest wall)**: 척추측만증(scoliosis) 등 근골격 이상으로 흉강이 충분히 확장되지 못하는 경우.
4. **흉막(pleura)**: 흉막 삼출(pleural effusion)이나 흉막 비후로 폐 확장이 제한되는 경우.
...
[▶ 영상으로 보기](https://youtu.be/jzTFHQDxcHI) *(멤버십)*

---
선행: [[resp-p10-pft-basics]]
