...
1) **ATP 2개(순생산)**: 4개 생성 — 2개 투자 = 순수익 2개
2) **NADH 2개**: 나중에 전자전달계(electron transport chain)로 들어가면 각각 약 2.5개의 ATP를 추가로 생산할 수 있다
...
1) 아세틸 CoA(acetyl-CoA)로 전환 → 시트르산 회로(citric acid cycle) 진입 (산소 있을 때)
2) 젖산(lactate)으로 전환 → 혈액으로 분비 (산소 없을 때)
3) 아미노산(amino acid)으로 전환 → 단백질 합성 재료

### 포도당 6인산(glucose-6-phosphate, G6P)의 운명

해당과정 첫 단계 산물인 G6P도 두 갈래로 나뉜다.
1) 글리코겐(glycogen) 합성 → 간과 근육에 에너지 저장
2) 오탄당 인산 경로(pentose phosphate pathway) → 핵산(DNA, RNA) 재료 공급
...
[▶ 영상으로 보기](https://youtu.be/4v0XV8QvWiA) *(멤버십)*

---
선행: [[glycolysis02-one-sentence]]
연관: [[glycolysis11-arsenic]]
다음: [[glycolysis11-arsenic]]
