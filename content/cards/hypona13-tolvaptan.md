...
1) G단백질 → 아데닐산 시클라아제(adenylyl cyclase) 활성화 → cAMP 증가 → PKA 활성화.
2) PKA가 세포 내에 있던 aquaporin-2를 세포막으로 이동시킨다.
3) 물이 aquaporin-2를 통해 재흡수된다(급성 반응).
...
저나트륨혈증 평가는 삼투압 확인 → 볼륨 상태 확인 → 원인 좁히기 순서로 진행된다. 치료는 단순하다. **만성이고 무증상이면 원인 치료**, **급성이고 증상이 있으면 3% 식염수**로 직접 교정한다. 교정 시에는 속도를 엄격히 제한해 ODS를 예방한다. tolvaptan은 수분 저류가 만성적으로 지속되는 상황에서 기존 치료의 한계를 보완하는 선택지다.

[▶ 영상으로 보기](https://youtu.be/h4Uv_Z-r4JE) *(멤버십)*

---
선행: [[hypona12-sodium-correction]], [[hypona-siadh]]
연관: [[hypona02-raas-adh-review]]
다음: [[hypona-treatment]]
